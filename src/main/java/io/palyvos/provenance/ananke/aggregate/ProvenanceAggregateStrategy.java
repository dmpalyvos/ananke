 ...
 * operators. Every {@link Accumulator} needs to have its own {@link
 * ProvenanceAggregateStrategy object} when constructing provenance graphs using {@link
 * #addWindowProvenance(GenealogTuple)} and {@link #annotateWindowResult(GenealogTuple)}. The traversal method {@link
 * #provenanceIterator(GenealogTuple)} acts as a factory for {@link Iterator}s and thus does not
 * require distinct {@link ProvenanceAggregateStrategy} objects for different traversals.
 */
public interface ProvenanceAggregateStrategy extends Serializable {
   ...
   * result of an aggregate operation (i.e., have {@link
   * GenealogTupleType} equal to {@link
   * GenealogTupleType#AGGREGATE}
   *
